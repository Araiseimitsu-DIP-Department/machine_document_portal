...
Googleスプレッドシートで運用している「稼働中工程内検査シート」を置き換え、号機ごとに現在品番、SharePoint上の工程内検査シートと数値検査用資料、NAS上の加工図をまとめて表示する社内向けWebアプリです。
...
- 工程内検査シートが複数ある場合は件数を表示し、同一タブの選択画面から全ファイルを開ける
- 出荷検査表と同じSharePointフォルダから、品番と完全一致するか、品番直後が `_`、`-`、半角・全角スペース、`・`の「数値検査用」資料を検索
- 各号機の資料欄を「検査シート」「加工図面」「数値検査用」の3列で表示し、資料の用途を識別できる個別アイコンを使用
- NASにある品番と同名のPDFの1ページ目を、ローカル配置したPDF.jsで専用タブへ高精細表示
- 加工図をボタン、マウスホイール、タッチ端末の2本指操作で50〜800%に拡大・縮小し、マウスドラッグで上下左右へ移動
...
- 号機一覧のブラウザタブ、ブックマーク、iOS／Androidのホーム画面追加、Web App Manifest（`display: browser`）の表示名を「稼働中工程内検査シート」に統一し、ホーム画面から通常ブラウザーで起動。加工図タブは「号機_品番_加工図面」で識別
...
号機一覧のブラウザタブ、ブックマーク、「ホーム画面に追加」の表示名は「稼働中工程内検査シート」に統一しています。Web App Manifestは`display: browser`とし、ホーム画面アイコンからも独立アプリ表示ではなく通常のブラウザー画面で起動します。加工図の専用タブだけは、開いている図面を識別できるよう「号機_品番_加工図面」と表示します。アイコンは `app/static/manifest.json` と `app/static/icons/` の画像を参照し、サイドバー上部の社名ロゴは `docs/DESIGN` の ARAI ロゴを使用します。アイコン画像を差し替えた場合は、各サイズを再生成し、`app/pwa.py` の `STATIC_ICONS_VERSION` を更新してください。CSS、JavaScript、Web App Manifestは内容から生成した `STATIC_ASSETS_VERSION` をURLへ付け、変更時にブラウザキャッシュを自動更新します。Service Workerは使用していないため、データ更新と画面の自動再読込は通常のブラウザ表示と同じです。

既に追加済みのホーム画面アイコンには以前の`standalone`設定が残る場合があります。Manifest変更後は、各端末で既存のホーム画面アイコンを一度削除し、SafariやChromeでページを再読込してから追加し直してください。Androidでは端末やブラウザーにより、インストール型アプリではなくブラウザーショートカットとして追加されます。

iPadで工程内検査シートを入力・保存しながら加工図を確認する場合は、ポータルとSharePoint／Excelを通常のブラウザー画面またはExcelアプリで開き、iPadOS標準のSplit Viewまたはステージマネージャで並べます。分割比率はiPadOSの仕切り線で調整してください。アプリ内へSharePoint編集画面を埋め込む方式は使用しません。
...
| --- | --- | --- |
| 1400px以上 | 5列 | 通常サイドバー |
| 1051〜1399px | 3列 | 通常サイドバー |
| 681〜1050px | 2列 | 900px以下ではアイコンサイドバー |
| 680px以下 | 1列 | 上部のメニューボタンから開くドロワー |

1400〜1699pxでは、1920×1080のWindows PCを表示倍率125%で利用した場合も5列を維持できるよう、3つの資料列を44px幅、列間を狭い余白で揃えます。グループヘッダーは色付きの英字だけを表示し、重複していた「〇グループ」「〇台」は画面上から省きます。資料見出しは文字サイズを下げず、「検査／シート」「加工／図面」「数値／検査用」と1行目が2文字になる位置で固定改行します。680px以下では号機グループを折りたため、初期表示は最初のグループだけを開きます。省略されやすい品番・品名は折り返して表示し、資料ボタンやメニューボタンは44px以上のタッチ領域を確保します。iPhone／iPadのセーフエリアと動的なブラウザ高さにも対応しています。
...
画面では「最新情報に更新」ボタンを必要以上に広げず、右側に最終更新日時、下側に「工程内検査シート・加工図面・数値検査用を更新するときに押してください。」という説明を1行で表示します。サイドバーには、利用者の操作判断に不要な内部保存方式や内部状態点を表示しません。外部連携エラーなど利用者が把握すべき状態は、画面上部の通知と資料ボタンで表示します。`DOCUMENT_REFRESH_TIMES=13:10` の場合は毎日13:10に完全同期し、`10:30,14:30` のような複数時刻も指定できます。空欄の場合は実行しません。
...
完全一致を最優先し、品番自体が`-1`などで終わる場合も省略しません。1件の場合は検査シートを直接別タブで開き、複数の場合は号機カードに件数を表示して、同一タブの選択画面へファイル名と保存場所をすべて表示します。
...
号機別の「数値検査用」は出荷検査表と同じSharePointフォルダを検索します。品番との完全一致、または品番直後が `_`、`-`、半角・全角スペース、`・`のファイルだけを対象にし、1件は直接別タブで開き、複数件は同一タブの専用一覧から開きます。翌営業日の13:00・14:30不足通知には含めません。
...
- ヘッダー中央に同じ配色の縮小・倍率リセット・拡大ボタンを配置し、50〜800%の範囲外へ進むボタンは無効化
- 表示領域の大きさを変えず、ボタン、マウスホイール、またはタッチ端末の2本指操作で内部図面だけを50〜800%に拡大・縮小
...
- Split ViewでもPDFの内部描画解像度は単体表示時の基準を維持し、PDF.js失敗時の代替JPEGも4倍解像度・品質95で生成
- Split Viewでも100%時の図面を単体表示時と同じ大きさで表示し、はみ出す範囲はスクロールして確認
...
- 工程内検査シートを入力・保存しながら加工図を確認できる画面分割（SharePointの編集画面を埋め込めるか、実端末での事前検証が必要）
- 加工図の複数ページ表示
