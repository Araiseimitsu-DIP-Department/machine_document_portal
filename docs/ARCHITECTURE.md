...
本番では `GoogleSheetsMemorySyncService` が「生産中」シートのA/D/H/I列を取得します。同じ同期処理で、工程内検査シートと数値検査用のSharePointフォルダ配下をそれぞれ再帰検索し、H列の品番をファイル名の拡張子を除いた文字列と正規化せず照合します。工程内検査シートは完全一致または`品番-[1-9][0-9]*`、数値検査用は完全一致または品番直後が `_`、`-`、半角・全角スペース、`・`の場合だけ関連ファイルとして扱います。NASは従来どおり同名PDFだけを検索します。D列に存在する号機だけで `MemoryDashboardStore` を置き換え、画面と更新APIは同じストアを共有します。
...
加工図の通常表示はPDFをそのままブラウザーへストリーミングするため、サーバー側で画像変換しません。PDF APIはメモリストアの現在品番を再確認して `NAS_DRAWING_DIRECTORY` 配下を完全一致検索し、NASの共有パスを公開せず、`application/pdf`、インライン表示、Range、ETag、Last-Modified、`private, max-age=3600` を返します。PDF.jsの失敗時だけ利用するJPEGプレビューはPDFの更新日時とサイズをキーに、最大64件かつ128MiBまでアプリ内へキャッシュします。同じPDFへの同時変換は1回にまとめます。

加工図表示は、「号機_品番_加工図面」形式の専用タブでPDFの1ページ目をPDF.js 5.7.284のCanvasへ描画します。PDF.js、Worker、CMap、標準フォント、WASMは同一オリジンからローカル配信し、Chrome／EdgeはModern版、SafariとAppleモバイル端末はLegacy版を使用します。PDF全体を先に取得する方式は、代表的な約2.2MiBの加工図でRangeによる複数往復より速かったため採用しています。HTTPキャッシュにより同じPDFの再取得を抑えます。

初回から端末の実ピクセル比で描画します。100%時の図面は単体表示時の大きさを基準とし、Split Viewでも図面全体を狭い領域へ縮小しません。表示領域からはみ出す部分はスクロールして確認します。ヘッダー中央のボタン、マウスホイール、タッチ端末の2本指操作で50〜800%に変更し、操作中はCSS transformで即応、停止250ms後に現在倍率で再描画します。PDFの内部描画解像度も端末の単体表示可能領域を基準に維持します。`ResizeObserver` と `visualViewport` により新規ウィンドウをSplit Viewへ移した際の表示領域変更を検知し、端末の実ピクセル比で再描画します。古い `RenderTask` はキャンセルし、新Canvasの描画完了後に差し替えて旧Canvasを解放します。表示領域からはみ出した図面はPointer Eventsとスクロール位置を使ってマウスの左ドラッグで上下左右へ移動します。

Canvas上限はPCが一辺5120px・1600万画素・DPR 2、タブレットが4096px・1600万画素・DPR 2、コンパクトなタッチ端末が3072px・600万画素・DPR 1.5です。PDF.js内部の画像用Canvasも端末別に制限します。PDF.js失敗時の代替JPEGは4倍・品質95で生成します。`100dvh`、セーフエリア、`touch-action: pan-x pan-y`でモバイルブラウザーへ追従します。`DEBUG=true` の場合はPDF.jsモジュール、PDF取得、初期化、初回描画、再描画、操作完了までの時間をコンソールとビューアーの `data-*` 属性へ記録します。
...
工程内検査シート候補は完全一致、連番の数値、ファイル名、相対フォルダの順、数値検査用候補は完全一致、ファイル名、相対フォルダの順で安定して並べます。候補が複数の場合、`DocumentState` に候補名・URL・相対フォルダを保持し、工程内検査シートは`/inspections/{machine_id}`、数値検査用は`/numeric-inspections/{machine_id}`の選択画面へ同一タブで遷移します。候補一覧からSharePointファイルを開く操作は別タブのままです。候補URLはHTTPまたはHTTPSだけを許可します。

サイドバーの工程内検査シート、出荷検査表、測定機器点検表は、`.env` に設定した共通URLをそのまま別タブで開く静的リンクです。数値検査用は出荷検査表と同じフォルダを検索しますが、サイドバーリンク自体は静的です。

## レスポンシブUI

号機一覧はCSS Gridで構成し、表示幅に応じて5列、3列、2列、1列へ切り替えます。各号機内は検査シート、加工図面、数値検査用の順に、44px幅の3資料列と狭い列間余白で配置します。資料見出しは文字サイズを維持したまま、「検査／シート」「加工／図面」「数値／検査用」の位置で固定改行します。検査シートは帳票、加工図面は三角定規付き帳票、数値検査用は「123」付き帳票のインラインSVGを使用して用途を区別します。901px以上は通常サイドバー、681〜900pxはアイコンだけのサイドバー、680px以下は上部メニューボタンとオフキャンバス式サイドバーを使用します。モバイルメニューを閉じている間は`aria-hidden`と`inert`で操作対象から外し、開いている間は背景スクロールと背面操作を抑止します。

号機グループは`details`／`summary`で折りたためる構造です。ヘッダーの視覚表示はグループ色の英字バッジだけとし、「〇グループ」「〇台」は重複表示を避けるためスクリーンリーダー専用テキストへ移します。PC・タブレットでは全グループを開き、680px以下では最初のグループだけを開いて初期スクロール量を抑えます。タッチ端末では品番・品名の省略表示を解除し、資料ボタンとモバイル操作ボタンは44px以上を確保します。
