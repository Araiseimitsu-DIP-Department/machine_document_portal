...
初回から端末の実ピクセル比で表示領域へ収まる解像度を描画します。ヘッダー中央のボタン、マウスホイール、タッチ端末の2本指操作で50〜800%に変更し、操作中はCSS transformで即応、停止250ms後に現在倍率で再描画します。タブレットではSplit Viewでの細部確認に備え、PDF.jsのCanvas解像度・メモリ上限を高めています。`ResizeObserver` と `visualViewport` により新規ウィンドウをSplit Viewへ移した際の表示領域変更を検知し、端末の実ピクセル比で再描画します。古い `RenderTask` はキャンセルし、新Canvasの描画完了後に差し替えて旧Canvasを解放します。表示領域からはみ出した図面はPointer Eventsとスクロール位置を使ってマウスの左ドラッグで上下左右へ移動します。

Canvas上限はPCが一辺5120px・1600万画素・DPR 2、タブレットが4096px・1000万画素・DPR 1.75、コンパクトなタッチ端末が3072px・600万画素・DPR 1.5です。PDF.js内部の画像用Canvasも端末別に制限します。`100dvh`、セーフエリア、`touch-action: pan-x pan-y`でモバイルブラウザーへ追従します。`DEBUG=true` の場合はPDF.jsモジュール、PDF取得、初期化、初回描画、再描画、操作完了までの時間をコンソールとビューアーの `data-*` 属性へ記録します。
