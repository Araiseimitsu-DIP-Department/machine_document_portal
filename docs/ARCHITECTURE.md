...
初回は表示領域へ収まる解像度で描画します。ヘッダー中央のボタン、マウスホイール、タッチ端末の2本指操作で50〜800%に変更し、操作中はCSS transformで即応、停止250ms後に現在倍率で再描画します。タブレットではSplit Viewでの細部確認に備え、PDF.jsのCanvas解像度・メモリ上限を高めています。古い `RenderTask` はキャンセルし、新Canvasの描画完了後に差し替えて旧Canvasを解放します。表示領域からはみ出した図面はPointer Eventsとスクロール位置を使ってマウスの左ドラッグで上下左右へ移動します。
