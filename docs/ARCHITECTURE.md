...
初回から端末の実ピクセル比で表示領域へ収まる解像度を描画します。ヘッダー中央のボタン、マウスホイール、タッチ端末の2本指操作で50〜800%に変更し、操作中はCSS transformで即応、停止250ms後に現在倍率で再描画します。Split ViewではCSS上の図面サイズだけを現在の表示領域へ合わせ、PDFの内部描画解像度は端末の単体表示可能領域を基準に維持します。`ResizeObserver` と `visualViewport` により新規ウィンドウをSplit Viewへ移した際の表示領域変更を検知し、端末の実ピクセル比で再描画します。古い `RenderTask` はキャンセルし、新Canvasの描画完了後に差し替えて旧Canvasを解放します。表示領域からはみ出した図面はPointer Eventsとスクロール位置を使ってマウスの左ドラッグで上下左右へ移動します。
