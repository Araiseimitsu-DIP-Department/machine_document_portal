...
- 号機一覧は1400px以上で5列、1051〜1399pxで3列、681〜1050pxで2列、680px以下で1列を基本とする。1400〜1699pxは資料列とボタンをコンパクトにして、1920×1080・表示倍率125%でもA/B列とC〜Fグループを横一列に表示する。
- Machine Document Portalの各号機は、品番表示の右へ「検査シート」「加工図面」「数値検査用」の順で44px幅の資料列を3つ配置する。文字やボタンを縮小せず、列間余白を詰めて一覧性を確保する。
- 資料列見出しは任意位置で折り返さず、「検査／シート」「加工／図面」「数値／検査用」の2行に固定し、1行目を2文字で揃える。
- グループヘッダーは色付き英字を主表示とし、「〇グループ」「〇台」の重複テキストは画面上へ表示しない。名称と台数は支援技術向けに保持する。
...
- 加工図ビューアはボタンと2本指操作による50〜800%の拡大・縮小、縦横スクロール、`100dvh`による表示高さ追従に対応する。タブレットのSplit Viewでは、PDFを高精細Canvasへ再描画して細部を確認できるようにする。
...
- Machine Document Portalの資料ボタンは、検査シートを罫線付き帳票、加工図面を三角定規付き帳票、数値検査用を「123」付き帳票で表し、電卓など用途を誤認しやすい図柄は使用しない。
